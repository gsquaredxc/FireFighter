...
async def check_if_can_delete(member, guild):
    return globalVars.gConfig.get_value(guild.id,"allow_delete_all") or await has_admin_perms_server(member, guild)


async def check_if_can_ban(member, guild):
    return globalVars.gConfig.get_value(guild.id,"allow_ban_all") or await has_admin_perms_server(member, guild)
